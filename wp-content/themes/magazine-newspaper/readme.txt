=== Magazine Newspaper ===
Contributors: thebootstrapthemes, freemius
Tags: news, blog, portfolio, grid-layout, one-column, two-columns, left-sidebar, right-sidebar, custom-colors, custom-background, custom-header, custom-logo, custom-menu, editor-style, featured-images, flexible-header, footer-widgets, full-width-template, rtl-language-support, style-variations, theme-options, translation-ready, threaded-comments

Description: Magazine Newspaper build for newspaper, media, magazine related project. This theme is a minimalist and fully responsive multipurpose theme with full WordPress Customizer integration, that enables you to customize your website with live preview

This theme is very lightweight and easy to use. This is one of  the best WordPress theme designed for newspaper, magazine, photographers, personal portfolio, travel blogging, travel bloggers, colleges, universities, small medium and large scale businesses, corporates, lawyers, bloggers, restaurants, weddings, hotels, churches, salons, fitness, resorts, ecommerce, businesses, creative freelancers, professionals etc. and also supports woocommerce. We provide you free wordpress theme that is SEO friendly and customizable according to your necessity and desire, which helps you make a positive impression on potential clients by showcasing your visual works in the best possible way.

Live Demo - https://thebootstrapthemes.com/preview/?demo=magazine-newspaper-pro
Theme Details - https://thebootstrapthemes.com/magazine-newspaper/
Theme Documentation - https://thebootstrapthemes.freshdesk.com/support/solutions/folders/44001184343



Requires at least: 5.0
Tested up to: 6.5
Stable tag: 3.5.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Magazine Newspaper is distributed under the terms of the GNU General Public License v2


== Installation ==
	
1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the magazine-newspaper.zip file. Click Install Now.
3. Click Activate to use your new theme right away.
4. Create a page "home" or Select the Page Tempalate Called Front Page 
5. Select static frontpage "home" from customizer or Page Created in Step 4
6. Create categories
7. Select category needed for the homepage

Magazine Newspaper WordPress Theme, Copyright 2023 The Bootstrap Themes
Magazine Newspaper is distributed under the terms of the GNU GPL


== Changelog ==

= 3.5.2 - February 2, 2025 =
* Default translations fixed

= 3.5.1 - February 2, 2025 =
* Default translations fixed

= 3.5.0 - June 21, 2024 =
* Import customizer settings from the free version
* Customizer styling

= 3.4.0 - April 26, 2024 =
* WordPress 6.5 compatibility tested.
* Colors and fonts adjusted.
* Plugin activation glitch fixed. 

= 3.3.2 - April 12, 2024 =
* Top menu overflow fixed.
* Breaking news vertical align fixed

= 3.3.1 - March 27, 2024 =
* Advertisement slider added.

= 3.3.0 - February 23, 2024 =
* Admin Dashboard page added.

= 3.2.1 - December 21, 2023 =
* Drag and Drop feature updated

= 3.2.0 - December 15, 2023 =
* WordPress 6.4 compatibility tested
* Product page URLs changed
* Added upgrade-to-pro control
* Footer copyright updated

= 3.1.3 - November 28, 2023 =
* Responsivity improved for top navigation menu

= 3.1.2 - October 11, 2023 =
* Compatibility tested with WordPress 6.3

= 3.1.1 - September 07, 2023 =
* added support to webp images.

= 3.1.0 - September 04, 2023 =
* added option to adjust colors of H2,H3,H4 headings

= 3.0.2 - August 27, 2023 =
* fixed bug in Demo Import plugin

= 3.0.1 - August 24, 2023 =
* fixed bug with Freemius free version

= 3.0.0 - August 24, 2023 =
* Kirki framework removed
* Optimization for mobile devices
* Clean and revised home sections
* Create new widget sections

= 1.1.2 - June 01, 2023 =
* Freemius SDK updated.

= 1.1.1 - April 07, 2022 =
* Single Post page image issue fixed.
* Breadcrumbs error fixed.

= 1.1.0 - March 03, 2022 =
* Freemius SDK updated.

= 1.0.9 - November 16, 2021 =
* Freemius license issue fixed.

= 1.0.8 - October 29, 2021 =
* Minor bug fixes.


= 1.0.7 - March 18, 2021 =
* Navigation issue fixed.


== Credits ==

* Based on Underscores http://underscores.me/, (C) 2012-2016 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)
* normalize.css http://necolas.github.io/normalize.css/, (C) 2012-2015 Nicolas Gallagher and Jonathan Neal, [MIT](http://opensource.org/licenses/MIT)
* Font-Awesome: fonts, License: SIL OFL 1.1
* Font-Awesome: css, License: MIT License
* Bootstrap: http://getbootstrap.com, License: MIT License
* Google Fonts - Apache License, version 2.0
* wp-bootstrap-navwalker – ​https://github.com/twittem/wp-bootstrap-navwalker, License: Distributed under the terms of the GPL-2.0+ @twittem
* trt-customizer-pro https://github.com/justintadlock/trt-customizer-pro, (C) 2016 © Justin Tadlock,  GNU GPL version 2 or later
* Wow.js: https://github.com/matthieua/WOW , License: MIT License
* Animate.css: https://github.com/daneden/animate.css , License: MIT License
* vTicker: http://richhollis.github.io/vticker/ , License: MIT License

